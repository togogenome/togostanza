class ProteinPfamPlotStanza < TogoStanza::Stanza::Base
  property :pfam_list do |tax_id, gene_id|
    ...
    results.each do |entity|
      ...
      pfam_name_list =  query("http://sparql-app.togogenome.org/sparql", <<-SPARQL.strip_heredoc)
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
  ...
  property :selected_tax_id do |tax_id, gene_id|
    tax_id =  query("http://sparql-app.togogenome.org/sparql", <<-SPARQL.strip_heredoc)
      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
      PREFIX skos: <http://www.w3.org/2004/02/skos/core#>
  ...
  resource :plot_data do |tax_id, gene_id|
    ...
    pfam_list = query("http://sparql-app.togogenome.org/sparql",<<-SPARQL.strip_heredoc)
      DEFINE sql:select-option "order"
      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
      PREFIX skos: <http://www.w3.org/2004/02/skos/core#>
      PREFIX up: <http://purl.uniprot.org/core/>
    ...
    query1 = Thread.new {
      habitat_list = query("http://sparql-app.togogenome.org/sparql",<<-SPARQL.strip_heredoc)
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    ...
    query2 = Thread.new {
      genome_list = query("http://sparql-app.togogenome.org/sparql",<<-SPARQL.strip_heredoc)
        DEFINE sql:select-option "order"
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX mccv: <http://purl.jp/bio/01/mccv#>
    ...
    query3 = Thread.new {
      summary_list = query("http://sparql-app.togogenome.org/sparql",<<-SPARQL.strip_heredoc)
        DEFINE sql:select-option "order"
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    ...
    pfam_list.each do |pfam_entity|
      ...
      pfam_summary_list = query("http://sparql-app.togogenome.org/sparql",<<-SPARQL.strip_heredoc)
        DEFINE sql:select-option "order"
