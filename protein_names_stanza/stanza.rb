class ProteinNamesStanza < TogoStanza::Stanza::Base
  property :protein_names do |tax_id, gene_id|
    # genes
    gene_names = query("http://dev.togogenome.org/sparql-app", <<-SPARQL.strip_heredoc)
    ...
    # summary
    protein_summary = query("http://dev.togogenome.org/sparql-app", <<-SPARQL.strip_heredoc)
      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
      PREFIX skos: <http://www.w3.org/2004/02/skos/core#>
      PREFIX up: <http://purl.uniprot.org/core/>
    ...
    organism = query("http://dev.togogenome.org/sparql-app", <<-SPARQL.strip_heredoc)
      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
