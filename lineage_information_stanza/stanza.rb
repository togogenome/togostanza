class LineageInformationStanza < TogoStanza::Stanza::Base
  ...
  property :tax_lineage_list do |tax_id|
    results_super = query("http://sparql-app.togogenome.org/sparql", <<-SPARQL.strip_heredoc)
    ...
    results_sub = query("http://sparql-app.togogenome.org/sparql", <<-SPARQL.strip_heredoc)
      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
      PREFIX taxo: <http://ddbj.nig.ac.jp/ontologies/taxonomy#>
      PREFIX taxid: <http://identifiers.org/taxonomy/>
